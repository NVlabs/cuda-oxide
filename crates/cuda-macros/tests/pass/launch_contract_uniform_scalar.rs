...
//! thread of the launch. A slice whose index space carries a runtime row pitch
//! is marshalled as `Pitched<T>`, which binds the pitch to that slice for the
...
/// `Pitched` for the slice that carries the row width.
fn host_signature_takes_the_bare_scalar_and_a_pitch(
    ...
    module.write_cells(stream, &prepared, 8u32, cuda_host::Pitched::new(out, 64))
}

fn main() {
    let _ = host_signature_takes_the_bare_scalar_and_a_pitch;
}
