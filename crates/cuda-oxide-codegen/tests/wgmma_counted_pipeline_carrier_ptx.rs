...
//! Proves that a two-slot BF16 WGMMA counted pipeline can keep sixty-four tied
//! `f32` accumulator values spill-free while loop control and affine descriptor
...
//! may add. The test isolates the harder backend question first:
//!
//! ```text
...
//! ```
//!
//! `ptxas -arch=sm_90a` must accept the generated PTX with zero stack frame and
//! zero spill traffic.
