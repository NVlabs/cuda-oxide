...
// the pitch of the slice being addressed. The flat accessors `get()` and
...
#[device]
pub fn bad_flat_accessors(pitched: DisjointSlice<u32, Runtime2DIndex>) {
    let idx = cuda_device::thread::index_2d_runtime(&pitched).unwrap();
    let _flat = idx.get();
    let _within = idx.in_bounds(pitched.len());
}
