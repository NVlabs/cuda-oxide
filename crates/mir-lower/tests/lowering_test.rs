...
#[test]
fn test_generated_ordered_sparse_mma_m16n8k64_int4_lowers_to_exact_convergent_inline_ptx()
-> Result<(), anyhow::Error> {
    use pliron::builtin::attributes::IntegerAttr;
    use pliron::builtin::types::{IntegerType, Signedness};
    use pliron::utils::apint::APInt;
    ...
    for backend in [
            ...
            cases.iter().enumerate()
        {
            ...
            let asm = &lowered[0];
            let expected_template = format!(
                "mma.sp::ordered_metadata.sync.aligned.m16n8k64.row.col{overflow_name}.s32.{a_name}.{b_name}.s32 {{$0, $1, $2, $3}}, {{$8, $9}}, {{$10, $11}}, {{$4, $5, $6, $7}}, $12, $13;"
            );
            assert_eq!(
                asm.get_attr_inline_asm_template(&ctx)
                    .as_deref()
