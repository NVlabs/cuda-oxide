...
pub(crate) fn convert_mma(
    ...
    pliron::input_err_noloc!(
        "WGMMA MMA reached lowering without deferred accumulator fusion; expected a supported BF16/F16 linear wait_group<0> region, a BF16 partial-wait pipeline, a BF16 canonical counted K-loop, or the BF16 fixed two-slot counted pipeline"
    )
}
