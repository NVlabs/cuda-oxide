...
mod kernels {
    use super::*;
    #[kernel]
    pub fn k_log(a: &[f32], mut out: DisjointSlice<f32>) {
        let idx = thread::index_1d();
        let i = idx.get();
        if let Some(o) = out.get_mut(idx) { *o = a[i].ln(); }
    }
}
fn main() {
    ...
    if d==0 { println!("k_log: BIT-IDENTICAL | {N}"); }
    else { println!("k_log: {d}/{N} differ, max ULP={mx} | {}",if mx<=2{"PASS"}else{"FAIL"}); }
}
