...
mod kernels {
    use super::*;
    #[kernel]
    pub fn k_selu(a: &[f32], mut out: DisjointSlice<f32>) {
        ...
        if let Some(o) = out.get_mut(idx) {
            ...
            *o = if x > 0.0f32 {
                ...
                1.05070098f32 * 1.67326324f32 * (x.exp() - 1.0f32)
            };
        }
    }
}
fn main() {
    ...
    let cpu: Vec<f32> = a.iter().map(|&x| if x > 0.0 {
        1.05070098f32 * x
    } else {
        1.05070098f32 * 1.67326324f32 * (x.exp() - 1.0f32)
    }).collect();
    ...
    m.k_selu(&s,LaunchConfig::for_num_elems(N as u32),&ad,&mut od).unwrap();
    ...
    if d==0 { println!("k_selu: BIT-IDENTICAL | {N}"); }
    else { println!("k_selu: {d}/{N} differ, max ULP={mx} | {}",if mx<=2{"PASS"}else if mx<=50{"allclose"}else{"FAIL"}); }
}
