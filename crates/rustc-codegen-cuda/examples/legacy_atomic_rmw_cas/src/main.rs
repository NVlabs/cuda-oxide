...
mod kernels {
    ...
    pub fn scoped_integer_cas(
        ...
        // Device scope normally stays as a native LLVM cmpxchg in lane 81A.
        // Stronger failure ordering forces the lane 81B PTX rewrite because
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let block_rmw_u32 =
        DeviceBuffer::<u32>::zeroed(&stream, 1)?.cast_elem::<BlockAtomicU32>();
    let block_rmw_u64 =
        DeviceBuffer::<u64>::zeroed(&stream, 1)?.cast_elem::<BlockAtomicU64>();
    let system_rmw_u32 =
        DeviceBuffer::<u32>::zeroed(&stream, 1)?.cast_elem::<SystemAtomicU32>();
    let system_rmw_u64 =
        DeviceBuffer::<u64>::zeroed(&stream, 1)?.cast_elem::<SystemAtomicU64>();
    ...
    let block_cas =
        DeviceBuffer::from_host(&stream, &[7_u32])?.cast_elem::<BlockAtomicU32>();
    let system_cas =
        DeviceBuffer::from_host(&stream, &[7_u32])?.cast_elem::<SystemAtomicU32>();
    ...
    {
        return Err(format!(
            "legacy scoped CAS mismatch: block={block_observed:?}/{block_cas_final}, system={system_observed:?}/{system_cas_final}, device={device_observed:?}/{device_cas_final}"
        )
        .into());
    }

    println!(
        "legacy_atomic_rmw_cas: PASS (device_rmw=({got_u32},{got_u64}), device_cas=({final_u32},{final_u64}), scoped_rmw=block({block_final_u32},{block_final_u64})/system({system_final_u32},{system_final_u64}), scoped_cas=block({block_cas_final})/system({system_cas_final})/device_strong_failure({device_cas_final}))"
    );
    Ok(())
}
