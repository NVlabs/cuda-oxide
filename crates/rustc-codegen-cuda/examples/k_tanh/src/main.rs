...
mod kernels {
    use super::*;
    #[kernel]
    pub fn k_tanh(a: &[f32], mut out: DisjointSlice<f32>) {
        let idx = thread::index_1d();
        let i = idx.get();
        if let Some(o) = out.get_mut(idx) {
            let x = a[i];
            // Use native .tanh() — this goes through std::sys::cmath::tanhf
            // Our collector.rs patch intercepts this before the FORBIDDEN CRATE panic
            *o = x.tanh();
        }
    }
}
fn main() {
    ...
    if d==0 { println!("k_tanh: BIT-IDENTICAL | {N}"); }
    else { println!("k_tanh: {d}/{N} differ, max ULP={mx} | {}",if mx<=2{"PASS"}else{"FAIL"}); }
}
