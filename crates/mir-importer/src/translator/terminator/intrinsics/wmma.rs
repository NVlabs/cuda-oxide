...
/// Emit `mma_m16n8k32_f8f6f4` as a register-producing dialect operation.
///
/// `atype` and `btype` are PTX type strings ("e4m3", "e5m2", "e3m2", "e2m3", "e2m1")
/// stored as StringAttr on the op. The lowering reads them to generate the
/// correct PTX mnemonic.
///
/// Args:
/// - `args[0]`: `[f32; 4]` C accumulator registers
/// - `args[1]`: `[u32; 4]` packed A fragment registers
...
/// Returns: `[f32; 4]` D accumulator registers.
#[allow(clippy::too_many_arguments)]
pub fn emit_mma_m16n8k32_f8f6f4(
    ctx: &mut Context,
    body: &mir::Body,
    args: &[mir::Operand],
    destination: &mir::Place,
    ...
    if args.len() != 3 {
        return input_err!(
            loc.clone(),
            TranslationErr::unsupported(format!(
                "mma_m16n8k32_f8f6f4 expects 3 arguments (acc, a, b), got {}",
                args.len()
            ))
        );
    }
    ...
    let mma_op = Operation::new(
        ctx,
        MmaM16N8K32F8F6F4Op::get_concrete_op_info(),
        vec![f32_ty.into(); 4],
        operands,
        vec![],
        0,
    ...
    // Set atype/btype as string attributes on the op.
    let typed = MmaM16N8K32F8F6F4Op::new(mma_op);
    typed.set_attr_mma_atype(ctx, StringAttr::new(atype.to_string()));
    typed.set_attr_mma_btype(ctx, StringAttr::new(btype.to_string()));
