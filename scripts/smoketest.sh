...
run_cargo() {
    ...
    if [[ "${cat}" == "blackwell-compile" ]]; then
        ...
        if [[ ! -s "${llvm_ptx}" ]] \
            ...
            printf 'direct LLVM route did not emit exact PTX 8.7/sm_120a with 25 f8f6f4, 4 FP8, 10 TF32, and 12 ldmatrix instructions\n' >>"${log}"
            if [[ ${VERBOSE} -eq 1 ]]; then
                printf 'direct LLVM route did not emit exact PTX 8.7/sm_120a with 25 f8f6f4, 4 FP8, 10 TF32, and 12 ldmatrix instructions\n'
            fi
            CARGO_EC=1
        ...
        if [[ ! -s "${nvvm_ll}" ]] \
            ...
            || grep -qE 'llvm\.nvvm\.(ff\.to\.|f2tf32|ldmatrix\.)' "${nvvm_ll}"; then
            printf 'libNVVM route did not emit exactly 4 FP8, 10 TF32, and 12 ldmatrix inline-PTX calls\n' >>"${log}"
            if [[ ${VERBOSE} -eq 1 ]]; then
                printf 'libNVVM route did not emit exactly 4 FP8, 10 TF32, and 12 ldmatrix inline-PTX calls\n'
            fi
            CARGO_EC=1
