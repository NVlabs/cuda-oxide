...
| `cmpxchg` | `i32`/`i64`, plus `i128` on `compute_90+`; global/shared pointers or generic pointers known to refer there | Kept native for scalar integers whose success/failure ordering pair libNVVM preserves; a stronger failure ordering is rewritten to inline PTX. Non-scalar-integer operands are rejected |
| `atomicrmw` | Integer `xchg`, `add`, `sub`, `and`, `or`, `xor`, `max`, `min`, `umax`, and `umin` on `i32`/`i64`; `i128 xchg` on `compute_90+`; the same address-space restriction | Kept native for `i32`/`i64` in the generic, global and shared address spaces, for exactly those ten operations, at monotonic ordering. Other widths, kinds, orderings and address spaces are rejected |
...
Two rules explain the shape of that column. The first is that nothing is
emitted whose type, address space, ordering and scope cannot be shown to
survive: #921 and #923 admitted integer RMW and compare-exchange by
proving those properties rather than by relaxing the requirement, which is why
the surviving rejections are the cases where the proof does not hold.
...
and a compare-exchange with a stronger failure ordering, are rewritten to
inline PTX that states the scope and ordering explicitly instead of being
handed to libNVVM. That rewrite needs `sm_70` or newer.
