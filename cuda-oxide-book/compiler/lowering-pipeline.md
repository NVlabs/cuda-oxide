...
is the minimal wiring (a hook rewrites a constant and the GPU result proves
the edited IR is what ran); the `enzyme_autodiff` example uses the hook to
differentiate a `#[device]` function with Enzyme.
