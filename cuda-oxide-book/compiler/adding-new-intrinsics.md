...
**File:** a hand-written module under `crates/dialect-nvvm/src/ops/`, as in
Stage 2 above. The warp shuffles are catalog intrinsics like `tid.x`, so the op
shown here is generated into `ops/generated/warp_shuffle.rs` rather than typed
out -- it is reproduced below because the shape is what matters here.
