...
| Warpgroup MMA (`wgmma`) | **Partial** | Hopper `sm_90a`: fence/commit/wait pipeline, shared-memory descriptors, `m64n64k16` BF16/F16 and `m64n64k8` TF32 with `f32` accumulation, plus m64n128 BF16 full-drain lowering. BF16 m64n64 supports full-drain, counted K-loops, and static partial waits; F16 m64n64 supports full-drain, counted K-loops, and the two-slot `wait_group<1>` straight-line pipeline; TF32 and m64n128 BF16 currently support full-drain only. |
